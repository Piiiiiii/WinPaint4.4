...
#define TIMER_BLINK500  2     /*500ms定时器事件标志号*/
#define TIMER_BLINK1000 3    /*1000ms定时器时间标志号*/
const int mseconds500 = 500;   
const int mseconds1000 = 1000; 
static bool isBlink = FALSE; 

double angle=0;//角度的全局变量 

static bool isDisplayLine = TRUE; 
static bool isDisplayRect = TRUE; 
static bool isDisplayEllipse = TRUE; 
static bool isDisplayText = TRUE;  
...
void DrawHouse1();/*画1号房子*/ 
void DrawHouse2();/*画2号房子*/ 
void littlebox();

void forward(double distance); // 沿当前方向画线，长度为distance，当distance为负数时后退
void turn (double a); // 顺时针旋转角度a，角度单位为DEG，当a为负数时逆时针旋转
...
static bool inText1 = FALSE; 
static bool inLine=FALSE;
static bool inRect=FALSE;
static bool inEllipse=FALSE;
	 	...
	 	case KEY_DOWN:
			 switch (key) {
			     case VK_F1:/*F1: 随机生成直线*/
					 ...
					 lptr->x1 = RandomReal(EDGE, GetWindowWidth());
				
					 lptr->y1 = RandomReal(0.0, GetWindowHeight());
					 lptr->x2 = RandomReal(EDGE, GetWindowWidth());
			     ...
			     case VK_F2:/*F2: 随机生成矩形*/
					 ...
					 x1 = RandomReal(EDGE, GetWindowWidth());
					 y1 = RandomReal(0.0, GetWindowHeight());
					 x2 = RandomReal(EDGE, GetWindowWidth());
			     ...
			     case VK_F3:/*F3: 随机生成椭圆*/
					 ...
					 eptr->cx = RandomReal(1.0+EDGE, GetWindowWidth()-1.0);
					 eptr->cy = RandomReal(1.0, GetWindowHeight()-1.0);
					 
					 do{
						eptr->rx = RandomReal(0.5, 3.0);
					 }while((eptr->cx-eptr->rx)<EDGE);
			     ...
			     case VK_F4:/*F4: 随机生成空的文本对象*/
					 ...
					 inText = TRUE;
					 inText1=TRUE;/*进入新的文本行输入状态*/
					 tptr = GetBlock(sizeof(*tptr));
					 tptr->x = RandomReal(1.0+EDGE, GetWindowWidth()-1.0);
...
void MouseEventProcess(int x, int y, int button, int event)
{
	...
	if(inText==TRUE)return;
 	if(inEllipse==FALSE&&inText1==FALSE&&inRect==FALSE&&inLine==FALSE) return;
	switch (event) {
...
void TimerEventProcess(int timerID)
{
	...
	switch (timerID) {
	    ...
	    case TIMER_BLINK1000:
		  ...
		  switch(curList){
		  	case LINE:
		  	SetEraseMode(isDisplayLine);
         	DrawLineD(lptr1);
	      	SetEraseMode(erasemode);
		  	isDisplayLine  = !isDisplayLine ;
		  	break;
		  	case RECT:
		  	SetEraseMode(isDisplayRect);
        	DrawRect(rptr1);
	      	SetEraseMode(erasemode);
		  	isDisplayRect = !isDisplayRect;
		  	break; 
		  	case ELLIPSE:
		  	SetEraseMode(isDisplayEllipse);
        	DrawCenteredEllipse(eptr1);
	      	SetEraseMode(erasemode);
		  	isDisplayEllipse = !isDisplayEllipse;
		  	break;
...
double x,y;             // 沿当前方向移动笔（不画线），长度为distance，当distance为负数时后退
 x=GetCurrentX()+distance*cos(angle);
 y=GetCurrentY()+distance*sin(angle); 
 MovePen(x, y);
} 

 void turn (double a){
                       
			 // 顺时针旋转角度a，角度单位为DEG，当a为负数时逆时针旋转
     angle-=a;
}
void DrawPolarLine(double r,double theta){
	double radians;
...
void other_imf(){
	int i,m,n; 
	MovePen(0.2, 6.6);
	DrawTextString("F1 随机生成直线");
	MovePen(0.2, 6.2);
	DrawTextString("F2 随机生成矩形");
	MovePen(0.2, 5.8);
	DrawTextString("F3 随机生成椭圆");
	MovePen(0.2, 5.4);
	DrawTextString("F4 随机生成空白文本");
	MovePen(0.2, 5);
	DrawTextString("F5 编辑文本");
	MovePen(0.2, 4.6);
	DrawTextString("F6 生成要求房屋图形");
	MovePen(0.2, 4.2);
	DrawTextString("F7 闪烁");
	MovePen(0.2, 3.8);
	DrawTextString("F8 生成雪花图形");
	MovePen(0.2, 3.0);
	DrawTextString("ESC 退出选中");
	MovePen(0.2, 2.6);
	DrawTextString("delete 删除");
	MovePen(0.2, 2.2);
	DrawTextString("enter 退出文本编辑状态");
	MovePen(0.2, 1.8);
	DrawTextString("3160102246");
	MovePen(0.2, 1.6);
	DrawTextString("樊子阳");
	MovePen(0.2, 1.2);
	DrawTextString("3160102301");
	MovePen(0.2, 1.0);
	DrawTextString("吴静怡");
	MovePen(0.2, 0.6);
	DrawTextString("3160102307");
	MovePen(0.2, 0.4);
	DrawTextString("刘天天");
} 
